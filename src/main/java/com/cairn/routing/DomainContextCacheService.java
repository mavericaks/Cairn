 ...
 * <p>
 * WHY: This acts as the external domain context cache between intent selection
 * and LLM execution (ADR-004). It isolates context per user and per domain.
 * <p>
 * RULE 16 ENFORCED: This service implements graceful degradation. If Redis goes offline,
 * the application will survive (treating queries as cache misses) rather than crashing
 * the entire semantic routing pipeline. It also records Micrometer metrics.
 */
@Service
public class DomainContextCacheService {
     ...
     * @return An Optional containing the context if it exists, empty otherwise.
     *         Returns empty if Redis is unreachable, allowing the system to degrade gracefully.
     */
    public Optional<String> getContext(String userId, String domain) {
        ...
        try {
            ...
            if (context != null) {
                ...
                meterRegistry.counter("cairn.cache.context", "result", "miss", "domain", domain).increment();
            ...
            log.error("Redis infrastructure failure while fetching context. Degrading gracefully to cache MISS. Key: [{}], Reason: {}", key, e.getMessage());
            return Optional.empty();
        } catch (Exception e) {
            meterRegistry.counter("cairn.cache.context", "result", "error").increment();
            log.error("Unexpected error accessing Redis context cache. Degrading to MISS. Key: [{}], Error: {}", key, e.getMessage(), e);
            return Optional.empty();
        }
    }
    ...
    public void saveContext(String userId, String domain, String context) {
        ...
        try {
            ...
            log.info("Saved context for user [{}] in domain [{}] with TTL {} hours", userId, domain, TTL.toHours());
        } catch (RedisConnectionFailureException | QueryTimeoutException e) {
            meterRegistry.counter("cairn.cache.save", "status", "failure").increment();
            log.error("Redis infrastructure failure while saving context. Proceeding without caching. Key: [{}], Reason: {}", key, e.getMessage());
            // WHY: Do not throw. Saving cache is not a critical path operation that should crash the user request.
        } catch (Exception e) {
            meterRegistry.counter("cairn.cache.save", "status", "error").increment();
            log.error("Unexpected error saving to Redis context cache. Proceeding without caching. Key: [{}], Error: {}", key, e.getMessage(), e);
        }
    }
    ...
    public void clearContext(String userId, String domain) {
        ...
        try {
            ...
            log.error("Redis infrastructure failure while clearing context. Key: [{}], Reason: {}", key, e.getMessage());
        } catch (Exception e) {
            meterRegistry.counter("cairn.cache.clear", "status", "error").increment();
            log.error("Unexpected error clearing Redis context cache. Key: [{}], Error: {}", key, e.getMessage(), e);
        }
    }
